...
def get_minecraft_bot() -> MinecraftBot:
    ...
    if (bot := get_bot(plugin_config.minecraft_bot_name)) is None:
        raise RuntimeError(f"名称为 {plugin_config.minecraft_bot_name} 的 Bot 实例未找到")
    elif not isinstance(bot, MinecraftBot):
        raise TypeError(f"名称为 {plugin_config.minecraft_bot_name} 的 Bot 实例不是 MinecraftBot 类型")
...
def get_qq_bot() -> OBBot:
    ...
    if (bot := get_bot(plugin_config.qq_bot_id)) is None:
        raise RuntimeError(f"名称为 {plugin_config.qq_bot_id} 的 Bot 实例未找到")
    elif not isinstance(bot, OBBot):
        raise TypeError(f"名称为 {plugin_config.qq_bot_id} 的 Bot 实例不是 QQ 类型")
